...
type SavedQuote = { quoteId: string; customerName: string; lines: CartLine[]; subtotal: number };
...
export function NewQuote({ catalog }: { catalog: InventoryItem[] }) {
  ...
  async function handleSave() {
    setError(null);
    setSaving(true);
    const result = await saveQuote(cart, customerName);
    setSaving(false);
    ...
    setSaved({ quoteId: result.quoteId, customerName: customerName.trim(), lines: cart, subtotal });
  }
  ...
  if (saved) {
    ...
    return (
      <Card className="mx-auto max-w-md">
        ...
        {saved.customerName && <p className="text-xs text-on-surface-variant">For: {saved.customerName}</p>}
